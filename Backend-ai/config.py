...
# --- Payment method --------------------------------------------------------
# Belongs to the sale, not the line item — a seller doesn't take half a sale
# in cash. "bank" covers KHQR scans, ABA/Wing/ACLEDA transfers, any non-cash.
# "credit" is not an edge case here — selling to regulars who pay later is
# normal trade. Recording it as cash would report money as received when it
# is still owed, which quietly inflates every daily total.
VALID_PAYMENT_METHODS = ("cash", "bank", "credit")

# Methods where the money is actually in hand. Anything outside this is a
# receivable, not income.
SETTLED_PAYMENT_METHODS = ("cash", "bank")

# Cash is overwhelmingly the default for small sellers, so an unspoken method
# is assumed rather than asked about. Flagged in the notes either way.
