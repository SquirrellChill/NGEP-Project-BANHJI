...
from app.core.config import settings, call_with_retry, generation_config, get_client
...
def _build_prompt(catalog: list | None) -> str:
    ...
    if names:
        prompt += prompts.CATALOG_HINT.format(
            catalog="\n".join(f"- {n}" for n in names[: settings.MAX_CATALOG_HINTS])
        )
    return prompt
...
def _transcribe_prompted(
    ...
    def call():
        return client.models.generate_content(
            model=settings.ASR_PROMPTED_MODEL,
            contents=[
                _audio_part(audio_bytes, mime_type),
                _build_prompt(catalog),
            ],
    ...
    response = call_with_retry(call, what="Transcription")
    return _response_text(response)
...
def _transcribe_endpoint(audio_bytes: bytes, mime_type: str) -> str:
    ...
    def call():
        return client.models.generate_content(
            model=settings.ASR_TRANSCRIBE_MODEL,
            contents=[_audio_part(audio_bytes, mime_type)],
            config=generation_config(),
        )
